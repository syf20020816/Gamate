...
import { Card, Select, InputNumber, Input, Radio, Switch, Button, Space, message } from 'antd';
...
export const SimulationPanel: React.FC = () => {
  const { config, setLivestreamConfig, addEmployee, updateEmployee, removeEmployee, startSimulation, stopSimulation } = useSimulationStore();
    ...
    <div className="simulation-panel">
      ...
      <Card title="直播间配置" size="small" style={{ marginBottom: 16 }}>
        <Space direction="vertical" style={{ width: '100%' }} size="middle">
          <div>
            <span style={{ marginRight: 8, minWidth: 80, display: 'inline-block' }}>在线人数:</span>
            <InputNumber
              min={1}
              max={10000}
              value={livestream.onlineUsers}
              onChange={(value) => setLivestreamConfig({ onlineUsers: value || 1000 })}
              style={{ width: 150 }}
            />
          </div>
          
          <div>
            <span style={{ marginRight: 8, minWidth: 80, display: 'inline-block' }}>直播间名称:</span>
            <Input
              value={livestream.roomName}
              onChange={(e) => setLivestreamConfig({ roomName: e.target.value })}
              placeholder="输入直播间名称"
              style={{ width: 300 }}
            />
          </div>
          ...
          <div>
            <span style={{ marginRight: 8, minWidth: 80, display: 'inline-block', verticalAlign: 'top' }}>直播间描述:</span>
            <TextArea
              value={livestream.roomDescription}
              onChange={(e) => setLivestreamConfig({ roomDescription: e.target.value })}
              placeholder="输入直播间描述"
              rows={3}
              style={{ width: 300 }}
            />
          ...
          <div>
            <span style={{ marginRight: 8, minWidth: 80, display: 'inline-block' }}>弹幕频率:</span>
            <Radio.Group
              value={livestream.danmakuFrequency}
              onChange={(e) => setLivestreamConfig({ danmakuFrequency: e.target.value })}
            >
          ...
          <div>
            <span style={{ marginRight: 8, minWidth: 80, display: 'inline-block' }}>礼物频率:</span>
            <Radio.Group
              value={livestream.giftFrequency}
              onChange={(e) => setLivestreamConfig({ giftFrequency: e.target.value })}
            >
              {FREQUENCY_OPTIONS.map(opt => (
                <Radio key={opt.value} value={opt.value}>{opt.label}</Radio>
              ))}
          ...
          <div>
            <span style={{ marginRight: 8, minWidth: 80, display: 'inline-block' }}>是否可上麦:</span>
            <Switch
      ...
      >
        <Space direction="vertical" style={{ width: '100%', overflowY: 'auto', height: 200 }} size="middle">
          ...
          {config.employees.map((employee, index) => (
            ...
            >
              <Space direction="vertical" style={{ width: '100%' }}>
                <div>
                  <span style={{ marginRight: 8, minWidth: 80, display: 'inline-block' }}>AI 性格:</span>
                  <Select
                    value={employee.personality}
                    onChange={(value) => updateEmployee(employee.id, { personality: value })}
                    style={{ width: 150 }}
                  >
                ...
                <div>
                  <span style={{ marginRight: 8, minWidth: 80, display: 'inline-block' }}>互动频率:</span>
                  <Radio.Group
                    value={employee.interactionFrequency}
                    onChange={(e) => updateEmployee(employee.id, { interactionFrequency: e.target.value })}
                  >
                    {FREQUENCY_OPTIONS.map(opt => (
                      <Radio key={opt.value} value={opt.value}>{opt.label}</Radio>
                    ))}
                ...
                <div>
                  <span style={{ marginRight: 8, minWidth: 80, display: 'inline-block' }}>AI 昵称:</span>
                  <Input
                    value={employee.nickname}
                    onChange={(e) => updateEmployee(employee.id, { nickname: e.target.value })}
                    placeholder="输入昵称"
                    style={{ width: 200 }}
                  />
                </div>
      ...
      <div style={{ textAlign: 'center', marginTop: 24 }}>
        <Button
          ...
          size="large"
          icon={config.isRunning ? <PauseCircleOutlined /> : <PlayCircleOutlined />}
