...
export const LivestreamHudPage: React.FC = () => {
  ...
  const {  messages, currentGame, isThinking, setCurrentGame, deleteMessage } = useAIAssistantStore();
  ...
  useEffect(() => {
    const setupListener = async () => {
      const unlisten = await listen<{ gameId: string }>('game-changed', (event) => {
  ...
  return (
    <div className="livestream-hud-page">
      ...
      <Card className="header-card" size="small">
        <div className="header-row">
          <div className="livestream-info">
            ...
            <Tag color="blue" icon={<UserOutlined />}>{livestream.onlineUsers} 在线</Tag>
          </div>
        </div>
      ...
      <div className="main-content">
        {/* 左侧: 弹幕对话窗 */}
        <Card className="chat-card" title={<><MessageOutlined /> 弹幕对话</>}>
          <ConversationArea
            messages={messages}
        ...
        {/* 右侧: AI 员工列表 */}
        <Card className="employees-card" title="AI 员工列表">
          <div className="employees-list">
            ...
            {config.employees.map((employee) => {
              ...
              return (
                <div key={employee.id} className={`employee-item ${isOnMic ? 'on-mic' : ''}`}>
                  <div className="employee-info">
                    <Badge
                      status={isOnMic ? 'success' : 'default'}
                      text={
                        <span style={{ color: isOnMic ? '#52c41a' : undefined }}>
                          {employee.nickname}
                        </span>
                      }
                    />
                    <Tag>{employee.personality === 'sunnyou_male' ? '损友男' : 
                          employee.personality === 'funny_female' ? '搞笑女' :
                          employee.personality === 'kobe' ? 'Kobe' :
                          employee.personality === 'sweet_girl' ? '甜妹' : '特朗普'}</Tag>
                  </div>
                  <Button
                    size="small"
