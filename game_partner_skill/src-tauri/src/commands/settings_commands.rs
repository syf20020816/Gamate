...
pub async fn save_app_settings(mut settings: AppSettings) -> Result<(), String> {
    ...
    if let Ok(existing_settings) = AppSettings::load() {
        ...
        if settings.simulation.employees.is_empty() && !existing_settings.simulation.employees.is_empty() {
            log::info!("🔒 保护模拟场景配置：保留 {} 个 AI 员工", existing_settings.simulation.employees.len());
            settings.simulation.employees = existing_settings.simulation.employees;
        }
        
        // 同样保护直播间配置（如果前端没有发送完整的直播间配置）
        if settings.simulation.livestream.room_name.is_empty() && !existing_settings.simulation.livestream.room_name.is_empty() {
            log::info!("🔒 保护直播间配置");
